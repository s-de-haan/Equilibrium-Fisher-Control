...
class oEWC_network(Network, FisherInterface):
    ...
    def get_regularization_strength(self):
        ...
        stats = {
            "fisher_norm": sum(torch.norm(f).item() ** 2 for f in self._fisher.values())
            ** 0.5,
            "fisher_mean": torch.cat([f.flatten() for f in self._fisher.values()])
            .mean()
            .item(),
            "fisher_max": torch.cat([f.flatten() for f in self._fisher.values()])
            .max()
            .item(),
        }
        return stats
