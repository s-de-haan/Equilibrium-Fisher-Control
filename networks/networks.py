...
class DFC_Mult_network(JacobianInterface):
    ...
    def _dynamical_inversion(self):
        ...
        e_psi_current = [torch.zeros((self.bzs, lod)) for lod in layer_out_dims]
        ...
        for _ in range(self.tmax - 1):
            ...
            for i, layer in enumerate(self.layers):
                ...
                # Apical input (e^Ju)
                e_psi_current[i] = torch.exp(torch.bmm(u_next.unsqueeze(1), Jis[i]).squeeze())

                # Soma with apical # TODO if last layer than -e_psi
                v_current[i] += (self.dt / self.time_constant_ratio) * (
                    e_psi_current[i] * v_ff_current[i] - v_current[i]
                )
...
class DFC_network(JacobianInterface):
    ...
    def _dynamical_inversion(self):
        ...
        prev_error = self.targets - r_current[-1]
        Js_init = torch.cat(Js, dim=-1)
        ...
        for _ in range(self.tmax - 1):
            ...
            #TODO: Test Broyden's method
            # modify the shape of the Js
            # _, Js = self._calculate_full_jacobian()

            _, Js = self._modified_broyden(Js_init, prev_error_vect=prev_error, current_error_vect=error, layer_sizes=self.layer_sizes)
            prev_error = error
